...
- **Auth rate limiting** — rate-limited by client IP; in-memory fixed-window store in `src/lib/rate-limit.ts` (single-instance only; resets on restart, not shared across replicas). Two enforcement points:
