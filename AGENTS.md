...
- [x] Invite-only registration (single-use per-person codes + a reusable multi-use "league invite" link with rotate/disable/cap — #110; the cap is enforced atomically under concurrent registrations — #123)
...
- [x] Settings page (first/last name + display name — same model as signup, #126 — email, reminders, password change; team management is admin-only)
...
- [x] Auto-grade picks — picks graded automatically when games go FINAL; a level game is a **push** (`PUSH`, 0 points, excluded from win% — #113)
